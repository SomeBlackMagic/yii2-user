...
$token = $I->grabRecord(Token::class, ['userId' => $user->id, 'type' => Token::TYPE_RECOVERY]);
...
$user = $I->grabFixture('user', 'user_with_expired_recoveryToken');
$token = $I->grabRecord(Token::class, ['userId' => $user->id, 'type' => Token::TYPE_RECOVERY]);
...
$user = $I->grabFixture('user', 'user_with_recoveryToken');
$token = $I->grabRecord(Token::class, ['userId' => $user->id, 'type' => Token::TYPE_RECOVERY]);
$I->amOnPage(Url::toRoute(['/user/recovery/reset', 'id' => $user->id, 'code' => $token->code]));
